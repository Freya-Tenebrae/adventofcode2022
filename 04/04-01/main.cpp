...
static bool included_in(int elf_a[2], int elf_b[2])
{
...
int main()
{
	...
	while(std::getline(input, line))
	{
		...
		if (included_in(elf_a, elf_b) || included_in(elf_b, elf_a))
			total++;
	}

	std::cout << "total of assignment pairs where one range fully contain the other : " << total << std::endl;
	return (0);
}